...
def discover_pricing():
    ...
    return jsonify({
        ...
        "avg_price_usd": round(sum(prices) / len(prices), 4) if prices else 0,
        "median_price_usd": sorted(prices)[len(prices) // 2] if prices else 0,
        "histogram": buckets,
...
def discover_openapi():
    ...
    spec = {
        ...
        "info": {
            "title": "AiPayGen API",
            "description": "169 AI tools with x402 USDC micropayments on Base Mainnet",
            "version": "2.0.0",
            "x-402-protocol": True,
...
def discover_compare():
    ...
    return jsonify({
        ...
        "our_advantages": [
            "169 AI-powered tools (not just proxies)",
            "Agent builder with scheduling",
            "MCP server with 169 tools",

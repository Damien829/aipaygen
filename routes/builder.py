...
def create_agent():
    ...
    if not isinstance(tools, list) or len(tools) > 169:
        return jsonify({"error": "tools must be a list of up to 169 tool names"}), 400
...
def update_custom_agent(agent_id):
    ...
    if "tools" in data:
        ...
        if len(tools) > 169:
            conn.close()
            return jsonify({"error": "maximum 169 tools allowed"}), 400

...
def test_model_count():
    assert len(MODEL_REGISTRY) >= 16  # grew from 16 with new model additions
...
def test_all_providers_have_streaming():
    ...
    providers_with_streaming = {"anthropic", "openai", "deepseek", "together", "xai", "mistral", "google", "ollama", "cohere", "perplexity"}
    for name, cfg in MODEL_REGISTRY.items():

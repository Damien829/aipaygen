...
_BOT_PATTERNS = _re.compile(
    r"(?i)(Googlebot|bingbot|Baiduspider|YandexBot|DotBot|AhrefsBot|SemrushBot|"
    r"MJ12bot|PetalBot|facebookexternalhit|Twitterbot|LinkedInBot|Slackbot|"
    r"Applebot|Bytespider|GPTBot|ClaudeBot|CCBot|DataForSeoBot|"
    r"ia_archiver|Sogoubot|Screaming Frog|Uptimerobot|pingdom|"
    r"bot|spider|crawler|headless|phantom|selenium)"
)
...
def init_funnel_db():
    with _conn() as c:
        ...
        # Add is_bot column if missing (migration for existing DBs)
        try:
...
def get_funnel_stats(days: int = 7, exclude_bots: bool = True) -> dict:
    ...
    bot_filter = " AND is_bot = 0" if exclude_bots else ""
    with _conn() as c:
        rows = c.execute(
